...
Do not repeat completed report prose.
Do not repeat sealed valid payloads.
Emit only missing or corrected frames and minimal report text.
Use the required report artifact for this role: %s.
Open with @report %s if a report frame is needed.
End with exactly one @result line whose artifact is %s.
